...
if (isset($_POST['request'])) {
    ...
    if ($documentType == "COG") {
        ...
        $html = str_replace(["{{dateOfRequest}}", "{{nameField}}", "{{semesterField}}", "{{programField}}", "{{schoolyearField}}", "{{author}}"], [$date, $fname . " " . $lname, $program, $semester, $schoolyear, $purpose, $author,], $html);
        $pdf->loadHtml($html);
        $pdf->render();
